...
class EventServiceLine(models.Model):
    ...
    def _check_date_to_date_from(self):
        ...
        for rec in self:
            ...
            if rec.event_id and rec.event_id.start_date and rec.date_from <= rec.event_id.start_date:
                raise ValidationError(_('Catering start date cannot be older than the event start date.'))
            if rec.event_id and rec.event_id.end_date and rec.date_to > rec.event_id.end_date:
                raise ValidationError(_('Catering end date cannot be greater than the event end date.'))
